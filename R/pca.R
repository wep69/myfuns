...
plot_pca_agri <- function(object,
  ...
  if (type == "scores") {
    map <- if (is.null(group_name)) {
      ...
      ggplot2::aes(x = .data[[pcx]], y = .data[[pcy]], shape = .data[[group_name]], group = .data[[group_name]])
    }
    p <- ggplot2::ggplot(scores, map) + ggplot2::geom_point(size = 2.4)
    if (isTRUE(ellipse) && !is.null(group_name)) p <- p + ggplot2::stat_ellipse()
    if (isTRUE(labels)) p <- p + ggplot2::geom_text(ggplot2::aes(label = .data[[".linha"]]), vjust = -0.7, show.legend = FALSE)
    return(p + labs + theme)
  }

  if (type == "loadings") {
    p <- ggplot2::ggplot(loads, ggplot2::aes(x = .data[[pcx]], y = .data[[pcy]])) +
    ...
    if (isTRUE(labels)) p <- p + ggplot2::geom_text(ggplot2::aes(label = .data[["variavel"]]), vjust = -0.6)
    return(p + labs + theme)
  }
  ...
  if (isTRUE(ellipse) && !is.null(group_name)) p <- p + ggplot2::stat_ellipse()
  if (isTRUE(labels)) p <- p + ggplot2::geom_text(ggplot2::aes(label = .data[[".linha"]]), vjust = -0.6, show.legend = FALSE)
  ...
  p +
    ggplot2::geom_segment(
      data = l2,
      mapping = ggplot2::aes(x = 0, y = 0, xend = .data[[pcx]], yend = .data[[pcy]]),
      inherit.aes = FALSE,
      arrow = grid::arrow(length = grid::unit(0.16, "cm"))
    ) +
    ggplot2::geom_text(
      data = l2,
      mapping = ggplot2::aes(x = .data[[pcx]], y = .data[[pcy]], label = .data[["variavel"]]),
