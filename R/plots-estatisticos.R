...
plot_emmeans <- function(object,
  ...
  p <- ggplot2::ggplot(est, ggplot2::aes(x = .data[[emm_x]], y = .data[[estimate_col]]))
  dodge <- ggplot2::position_dodge(width = 0.35)
  if (!is.null(group_var)) {
    p <- p + ggplot2::aes(group = .data[[group_var]], shape = .data[[group_var]])
  }
  ...
  if (!is.null(data) && isTRUE(show_data)) {
    ...
    raw_map <- if (is.null(group_var) || !group_var %in% names(data)) {
      ...
      ggplot2::aes(x = .data[[x_name]], y = .data[[y_name]], shape = .data[[group_var]], group = .data[[group_var]])
    }
  ...
  p <- p +
    ggplot2::geom_errorbar(
      ggplot2::aes(ymin = .data[[ci[1L]]], ymax = .data[[ci[2L]]]),
  ...
  if (isTRUE(letters) && !is.null(cld) && ".group" %in% names(cld)) {
    ...
    p <- p + ggplot2::geom_text(
      data = cld,
      mapping = ggplot2::aes(x = .data[[lx]], y = .data[[le]], label = trimws(.data[[".group"]])),
...
plot_reg <- function(object,
  ...
  if (isTRUE(show_means)) {
    means <- stats::aggregate(data[[y_name]], by = list(x = data[[x_name]]), FUN = mean)
    names(means) <- c(x_name, "media")
    p <- p + ggplot2::geom_point(
      data = means,
      mapping = ggplot2::aes(x = .data[[x_name]], y = .data[["media"]]),
      size = 3
    )
  }
  if (identical(interval, "confidence")) {
    p <- p + ggplot2::geom_ribbon(
      data = pred,
      mapping = ggplot2::aes(x = .data[[object$x]], ymin = .data[["ic_inferior"]], ymax = .data[["ic_superior"]]),
  ...
  p <- p +
    ggplot2::geom_line(
      data = pred,
      mapping = ggplot2::aes(x = .data[[object$x]], y = .data[["ajustado"]]),
      linewidth = 0.9
    ) +
